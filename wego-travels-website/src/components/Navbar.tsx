...
export function Navbar() {
  ...
  return <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${navBackground}`}>
    ...
    >
      ...
      <div className="flex-1 px-6 space-y-2 overflow-y-auto">
        ...
        <Link
          to="/contact"
          onClick={() => setIsMobileMenuOpen(false)}
          className={`block py-6 px-8 text-center rounded-2xl font-bold uppercase tracking-widest transition-all ${isContactActive ? 'bg-[#0167B2] text-white shadow-xl' : 'bg-orange-500 text-white hover:bg-orange-600 shadow-lg'
            }`}
        >
          Contact Us
        </Link>
